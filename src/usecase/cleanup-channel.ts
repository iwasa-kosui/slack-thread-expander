...
const deleteOne =
  (deps: CleanupChannelDeps, label: string, channel: ChannelId) =>
  (state: DeleteLoopState, ts: SlackTs): DeleteLoopState => {
    ...
    deps.logger.warn(`${label} failed to delete ts=${ts}: ${SlackApiError.format(res.error)}`);
    return { ...state, failures: [...state.failures, { ts, error: res.error }] };
  };

export const cleanupChannel =
  (deps: CleanupChannelDeps) =>
  (channel: ChannelId, selfBotId: BotId | undefined): ChannelCleanupOutcome => {
    const label = `[${channel}]`;

    if (selfBotId == null) {
      deps.logger.warn(`${label} SELF_BOT_ID is not configured; cleanup requires it to identify the bot's own posts`);
      return { kind: 'SkippedMissingSelfBotId', channel };
    }

    const listed = deps.slack.listChannelBotMessages({ channel, botId: selfBotId });
    if (Result.isFailure(listed)) {
      deps.logger.warn(`${label} conversations.history failed: ${SlackApiError.format(listed.error)}`);
      return { kind: 'ListFailed', channel, error: listed.error };
    }

    const { ts, truncated } = listed.value;
    deps.logger.info(`${label} cleanup target: scanned=${ts.length}${truncated ? ' (truncated)' : ''}`);
